...
export const calculateCitiesDistance = (origins, destinations, travelMode) => dispatch => {
  dispatch({ type: GET_DISTANCE_START })
  return axios.post('http://localhost:3355/users/match/distance', { origins, destinations, travelMode })
    .then(res => {
      dispatch({ type: GET_DISTANCE_SUCCESS, payload: res.data })
    })

    .catch(err => {
      dispatch({ type: GET_DISTANCE_FAILURE, payload: err })
      console.log(`unable to get distance, ${err}`)
    })
}
